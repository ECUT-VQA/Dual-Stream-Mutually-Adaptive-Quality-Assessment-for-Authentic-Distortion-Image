...
class DataLoader(object):
    ...
    def __init__(self, dataset,img_size,batch_size=1, istrain=True,isval=True,num_workers=4):
        ...
        if dataset == 'live':
            ...
            X_train, X_test = create_WVC3D1_data()
            self.train_dataset,self.test_dataset= create_WVC3D1_dataset(X_train, X_test, img_size)
        elif dataset=='WVC3D2':
            X_train, X_test = create_WVC3D2_data()
            self.train_dataset,self.test_dataset= create_WVC3D2_dataset(X_train, X_test, img_size)
    def get_data(self):
        if self.istrain:
        ...
        return dataloader
def create_WVC3D2_dataset(data_dir,train_data,test_data,image_size):
    ...
    train_dataset= WVC3D2_dataset(data_dir,train_data, transform=data_transform["train"])
    test_dataset = WVC3D2_dataset(data_dir, test_data, transform=data_transform["val"])
    return train_dataset,test_dataset
def create_WVC3D1_dataset(data_dir,train_data,test_data,image_size):
    ...
    train_dataset= WVC3D1_dataset(data_dir,train_data, transform=data_transform["train"])
    test_dataset = WVC3D1_dataset(data_dir, test_data, transform=data_transform["val"])
    return train_dataset,test_dataset
class WVC3D2_dataset(torch.utils.data.Dataset):
    def __init__(self, dir, data,transform=None,choice=False):
        self.data = data
        self.transform = transform
        self.mydir = dir
        self.choice=choice
        self.df_left = pd.read_excel('D:/dataset/WaterlooIVC3DPhaseII/2DLeft/MOS.xlsx', header=None)
        self.df_left = self.df_left.set_index(self.df_left.columns[0])
        self.df_right=pd.read_excel('D:/dataset/WaterlooIVC3DPhaseII/2DRight/MOS.xlsx',header=None)
        self.df_right = self.df_right.set_index(self.df_right.columns[0])
    # dataset length
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        img_left = img.crop((0, 0, split_position, height))
        img_right = img.crop((split_position, 0, width, height))
        img_left = self.transform(img_left)
        img_right = self.transform(img_right)

        left_label =self.df_left.loc[self.data.iloc[:,0][idx][:-4]+'Left'+self.data.iloc[:,0][idx][-4:-2]][3]
        # print(self.data.iloc[:,0][idx][:-4]+'Left'+self.data.iloc[:,0][idx][-4:-2])
        right_label=self.df_right.loc[self.data.iloc[:,0][idx][:-4]+'Right'+self.data.iloc[:,0][idx][-2:]][3]
        # print(self.data.iloc[:,0][idx][:-4]+'Right'+self.data.iloc[:,0][idx][-2:])
        label=self.data.loc[idx][2]

        return img_left,img_right, left_label, right_label,label
class WVC3D1_dataset(torch.utils.data.Dataset):
    def __init__(self, dir, data,transform=None,choice=False):
        self.data = data
        self.transform = transform
        self.mydir = dir
        self.choice=choice
        self.df_left = pd.read_excel('D:/dataset/WaterlooIVC3DPhaseI/2DLeft/MOS.xlsx', header=None)
        self.df_left = self.df_left.set_index(self.df_left.columns[0])
        self.df_right=pd.read_excel('D:/dataset/WaterlooIVC3DPhaseI/2DRight/MOS.xlsx',header=None)
        self.df_right = self.df_right.set_index(self.df_right.columns[0])
    # dataset length
    def __len__(self):
        self.filelength = len(self.data)
    ...
    def __getitem__(self, idx):
        ...
        return img_left,img_right, left_label, right_label,label
def create_WVC3D2_data():
    # 读取XLSX文件并将其转换为DataFrame
    df = pd.read_excel('D:/dataset/WaterlooIVC3DPhaseII/3D/MOS.xlsx', header=None)
    # 设置文件夹路径
    folder_path = 'D:/dataset/WaterlooIVC3DPhaseII/3D/Session3DIQ/'  # 将'path_to_folder'替换为实际的文件夹路径
    # 读取文件夹下的文件名
...
def create_WVC3D1_data():
    # 读取XLSX文件并将其转换为DataFrame
    df = pd.read_excel('D:/dataset/WaterlooIVC3DPhaseI/3D/MOS.xlsx', header=None)
    # 设置文件夹路径
    folder_path = 'D:/dataset/WaterlooIVC3DPhaseI/3D/Session3DIQ/'  # 将'path_to_folder'替换为实际的文件夹路径
    # 读取文件夹下的文件名
    image_names = [f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))]
    df2 = pd.DataFrame({'image_name': image_names})
    result = pd.concat([df, df2], axis=1)
